...
def test_works_without_parameters_using_env_default(capsys):
    ...
    with freeze_time("2023-01-01"):
        ...
        assert not event.is_anonymized
        ...
        assert event.is_anonymized is True
...
def test_respects_env_var_for_retention_days(capsys):
    ...
    with freeze_time("2023-01-01"):
        ...
        assert old_event.is_anonymized is True
        assert recent_event.is_anonymized is False  # Unchanged
...
def test_cli_overrides_env_var(capsys):
    ...
    with freeze_time("2023-01-01"):
        ...
        assert event_20d.is_anonymized is True
        assert event_10d.is_anonymized is False
...
def test_dry_run_does_not_change_data(capsys):
    ...
    with freeze_time("2023-01-01"):
        ...
        assert event.is_anonymized is False
...
def test_idempotency_already_anonymized_events_unchanged(capsys):
    ...
    with freeze_time("2023-01-01"):
        ...
        # Mark as already anonymized
        event.is_anonymized = True
        event.save()

        call_command("obfuscate_events")

        # Should remain anonymized without changes
        event.refresh_from_db()
        assert event.is_anonymized is True
        # Should not have been processed again
        assert event.name != "Anonymized event"  # Original name preserved
...
def test_already_anonymized_events_are_skipped(capsys):
    ...
    with freeze_time("2023-01-01"):
        ...
        # Mark one as already anonymized
        event1.is_anonymized = True
        event1.save()
        ...
        assert event1.is_anonymized is True
        assert event1.name != "Anonymized event"  # Original name preserved
        assert event2.is_anonymized is True
